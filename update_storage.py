...
# 1. We replace the fetchNearby function
old_fetchNearby_pattern = r"async function fetchNearby\(lat: number, lon: number, radiusKm: number\): Promise<ColdStorage\[\]> \{.*?\n\}"
...
async function fetchNearby(lat: number, lon: number, radiusKm: number): Promise<{results: ColdStorage[], isFallback: boolean}> {
  ...
  if (results.length === 0) {
    ...
    const fallbackWithDist = FALLBACK_STORAGES.map(s => {
      ...
      return { ...s, distanceKm: d, mapsUrl: https://www.google.com/maps/search/?api=1&query=, };
    });
...
# Wait, we can just replace everything between 'async function fetchNearby' and 'const [selectedId, setSelectedId] = useState'
start_idx = content.find('async function fetchNearby(lat: number, lon: number, radiusKm: number): Promise<ColdStorage[]> {')
end_idx = content.find('  const runSearch = useCallback(async (lat: number, lon: number, name: string) => {')

if start_idx != -1 and end_idx != -1:
    end_idx -= 2 # keep the spaces
    content = content[:start_idx] + new_fetchNearby + content[end_idx:]
    
    # We also need to update runSearch to use this new signature
    ...
    new_runSearch = '''  const [isFallbackData, setIsFallbackData] = useState(false);
  ...
  const runSearch = useCallback(async (lat: number, lon: number, name: string) => {
    ...
    # Also we should show a message if isFallbackData is true
    old_message = '''              {results.length === 0 ? (
                <div style={{ padding: '60px 20px', textAlign: 'center', background: '#fff', borderRadius: 16 }}>
                  <AlertCircle size={40} style={{ color: '#9ca3af', margin: '0 auto 16px' }} />
                  <p style={{ fontSize: '1.1rem', fontWeight: 600, color: '#374151', margin: '0 0 8px 0' }}>No facilities found nearby</p>
                  <p style={{ color: '#6b7280', margin: 0, fontSize: '0.95rem' }}>Try expanding your search radius up to 200km or search for a major city.</p>
                </div>
              ) : ('''
              
    # wait, if results is empty it will fall back to the 3 closest, so results.length is never 0!
    # But just in case, we can add a banner before the results list if isFallbackData is true.
    old_banner = '''                <div style={{ marginTop: 20, background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: 12, padding: '14px 18px', display: 'flex', gap: 10 }}>
                  <Info size={16} style={{ color: '#0ea5e9', flexShrink: 0, marginTop: 2 }} />
                  <p style={{ margin: 0, fontSize: '0.82rem', color: '#075985', lineHeight: 1.5 }}>
                    Results are from <strong>OpenStreetMap (Overpass API)</strong>'''
                    
    new_banner = '''                {isFallbackData && (
                  <div style={{ marginBottom: 16, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: '14px 18px', display: 'flex', gap: 10 }}>
                    ...
                    <p style={{ margin: 0, fontSize: '0.9rem', color: '#92400e', lineHeight: 1.5 }}>
                      No cold storages found within {radiusKm}km on OpenStreetMap. Showing the nearest available major facilities instead, which may be further away.
                    </p>
                  </div>
                )}
