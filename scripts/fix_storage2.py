...
bad_query = r'async function fetchNearby\(lat: number, lon: number, radiusKm: number\): Promise<\{results: ColdStorage\[\], isFallback: boolean\}> \{.*?return \{ results, isFallback \};\n\}'

good_query = """async function fetchNearby(lat: number, lon: number, radiusKm: number): Promise<{results: ColdStorage[], isFallback: boolean}> {
